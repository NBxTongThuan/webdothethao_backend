...
public class AdminNotificationController {
    ...
    public ResponseEntity<PagedModel<EntityModel<NotificationsResponse>>> findUnReadNotification(
            @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "1") int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdDate"));
        Page<NotificationsResponse> notificationsResponses =
                notificationService.getUnReadNotifications(pageable).map(NotificationsResponse::new);
        PagedModel<EntityModel<NotificationsResponse>> pagedModel =
                notificationsResponsePagedResourcesAssembler.toModel(notificationsResponses);
        return ResponseEntity.ok(pagedModel);
    }

    @GetMapping("/get-all")
    public ResponseEntity<PagedModel<EntityModel<NotificationsResponse>>> findAllNotification(
            @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "1") int size) {
        Pageable pageable = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdDate"));
        Page<NotificationsResponse> notificationsResponses =
                notificationService.findAll(pageable).map(NotificationsResponse::new);
        PagedModel<EntityModel<NotificationsResponse>> pagedModel =
                notificationsResponsePagedResourcesAssembler.toModel(notificationsResponses);
        return ResponseEntity.ok(pagedModel);
    }
