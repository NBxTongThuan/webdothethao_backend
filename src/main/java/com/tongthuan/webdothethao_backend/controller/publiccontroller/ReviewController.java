...
public class ReviewController {
    ...
    public ResponseEntity<List<GetReviewResponse>> getListReviewsByProductID(
            @RequestParam("productId") String productId) {
        if (productId.equalsIgnoreCase("")) return ResponseEntity.badRequest().build();
        return ResponseEntity.ok()
                .body(reviewsService.findByProductId(productId).stream()
                        .map(GetReviewResponse::new)
                        .toList());
    }
